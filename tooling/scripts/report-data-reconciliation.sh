...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c \
  "select OPERATION_ID, ORDER_NUMBER, OPERATION_TYPE, REFUND_ID, ATTEMPTS, LAST_ERROR, CREATION_DATE from ${DB_SCHEMA}.PAYMENT_RECONCILIATION_OPERATION where STATUS = 'MANUAL_REVIEW' order by CREATION_DATE, OPERATION_ID"
