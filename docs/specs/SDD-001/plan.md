...
`tooling/agent-harness/harness.py` owns validation, scheduling, feature/protocol fingerprints, immutable task packets, atomic task state, bounded rework/escalation, risk-to-reviewer routing and isolated Git worktrees.
...
`tooling/agent-harness/verification_sandbox.py` adds isolation around deterministic verification after runner allowlisting:
...
`tooling/agent-harness/control_plane.py` provides optional GitHub/Jira intake. It may perform HTTPS GETs and normalize/save/render local snapshots, but exposes no remote-write action. Tracker text is labeled untrusted intent context and must pass through normal specification/clarification before it can become an accepted spec.
...
`tooling/agent-harness/wayfinder.py` sits before the normal feature spec for efforts whose destination is known but route is genuinely foggy/multi-session. A durable `docs/wayfinder/<epic>/wayfinder.json` stores the destination, out-of-scope boundary, known fog and decision tickets; detailed closed-decision evidence lives under `decisions/`. Runtime claims/logs remain ignored under `.agent-state/.agent-runs`.
...
| Area | Verification |
|---|---|
| DAG/schema/AC/profile/path/lease rules | `python3 -m unittest tooling/agent-harness/tests/test_harness.py -v` |
| Provider boundary + terminal provenance behavior | `python3 -m unittest tooling/agent-harness/tests/test_runner.py -v` |
| Parallel orchestration/heartbeat/run-scoped ownership | `python3 -m unittest tooling/agent-harness/tests/test_orchestrate.py -v` |
| Verification sandbox modes | `python3 -m unittest tooling/agent-harness/tests/test_verification_sandbox.py -v` |
| Provenance/usage/known-cost aggregation | `python3 -m unittest tooling/agent-harness/tests/test_telemetry.py -v` |
| Read-only tracker normalization/intake | `python3 -m unittest tooling/agent-harness/tests/test_control_plane.py -v` |
| Grill/prototype design loop and hash-bound gate | `python3 -m unittest tooling/agent-harness/tests/test_design.py tooling/agent-harness/tests/test_harness.py -v` |
| Wayfinder map/frontier/claims/handoff | `python3 -m unittest tooling/agent-harness/tests/test_wayfinder.py -v` |
| Whole protocol | `python3 -m unittest discover -s tooling/agent-harness/tests -v` |
| All committed specs/profiles | `python3 tooling/agent-harness/harness.py validate-all docs/specs` |
| Local environment | `python3 tooling/agent-harness/harness.py doctor` |
| Orchestration plan | `python3 tooling/agent-harness/orchestrate.py docs/specs/SDD-001 --plan` |
| Maintained design previews | `python3 tooling/agent-harness/design.py docs/agentic-sdd/examples/INV-LOW-001 --plan` and `.../INV-CONTENTION-001 --plan` |
