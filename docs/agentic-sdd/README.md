...
```bash
python3 tooling/agent-harness/verification_contract.py generate docs/specs/<FEATURE> \
  --provider claude

python3 tooling/agent-harness/verification_contract.py validate docs/specs/<FEATURE>
```
...
```bash
python3 tooling/agent-harness/eval.py compare <old-result.json> <new-result.json>
```
...
python3 tooling/agent-harness/wayfinder.py run docs/wayfinder/SHIP-PLATFORM-001 \
...
python3 tooling/agent-harness/wayfinder.py to-tasks docs/specs/SHIP-PLATFORM-001 \
  --provider codex --reasoning high
...
python3 -m unittest discover -s tooling/agent-harness/tests -v
...
python3 tooling/agent-harness/design.py docs/specs/INV-LOW-001 --plan
```

Run the live design loop before implementation:

```bash
python3 tooling/agent-harness/design.py docs/specs/INV-LOW-001 \
  --provider codex \
  --reasoning high
```
...
```bash
python3 tooling/agent-harness/harness.py validate docs/specs/INV-LOW-001
python3 tooling/agent-harness/orchestrate.py docs/specs/INV-LOW-001 --plan
```
...
```bash
python3 tooling/agent-harness/orchestrate.py docs/specs/INV-LOW-001 \
  --provider codex \
  --reasoning high \
...
```bash
python3 tooling/agent-harness/harness.py status docs/specs/INV-LOW-001
python3 tooling/agent-harness/telemetry.py --feature INV-LOW-001
git worktree list
```
...
```bash
python3 tooling/agent-harness/harness.py reset docs/specs/INV-LOW-001 --full
```
...
`tooling/agent-harness/design.py` is deliberately separate from task orchestration. It requires only `spec.md`, `plan.md`, and optionally
...
```bash
python3 tooling/agent-harness/design.py docs/specs/SHOP-001 --plan
```

Run with Codex:

```bash
python3 tooling/agent-harness/design.py docs/specs/SHOP-001 \
  --provider codex \
  --reasoning high
```
...
python3 tooling/agent-harness/design.py docs/specs/INV-CONTENTION-001 --provider codex --reasoning high
```
...
```bash
python3 tooling/agent-harness/harness.py validate docs/specs/SHOP-001
```

Show tasks whose dependencies are satisfied:

```bash
python3 tooling/agent-harness/harness.py ready docs/specs/SHOP-001
```

Show specialist reviewers selected for one task:

```bash
python3 tooling/agent-harness/harness.py reviewers docs/specs/SHOP-001 T-002
```

Preview full scheduling/provider routing without changing state:

```bash
python3 tooling/agent-harness/orchestrate.py docs/specs/SHOP-001 --plan
```

Execute the DAG:

```bash
python3 tooling/agent-harness/orchestrate.py docs/specs/SHOP-001 \
  --provider codex \
  --review-provider claude \
  --evaluator-provider claude \
  --reasoning high
...
```bash
python3 tooling/agent-harness/harness.py status docs/specs/SHOP-001
...
```bash
python3 tooling/agent-harness/harness.py packet docs/specs/SHOP-001 T-002
python3 tooling/agent-harness/harness.py claim docs/specs/SHOP-001 T-002 --owner codex-1
python3 tooling/agent-harness/harness.py heartbeat docs/specs/SHOP-001 T-002 --owner codex-1
python3 tooling/agent-harness/harness.py complete docs/specs/SHOP-001 T-002 \
  --owner codex-1 \
...
```bash
python3 tooling/agent-harness/harness.py heartbeat docs/specs/SHOP-001 T-002 --owner codex-1
python3 tooling/agent-harness/harness.py recover-stale docs/specs/SHOP-001
```
...
python3 tooling/agent-harness/harness.py human-resolve docs/specs/SHOP-001 T-002 \
...
execution. Commands must pass the allowlist and then run through `tooling/agent-harness/verification_sandbox.py`.
...
```bash
python3 tooling/agent-harness/telemetry.py
python3 tooling/agent-harness/telemetry.py --feature SHOP-001
python3 tooling/agent-harness/telemetry.py --feature SHOP-001 --orchestration-id <run-id>
```
...
```bash
python3 tooling/agent-harness/control_plane.py github \
  --repo chrobakpiotr/showcase-application --issue 123 --intent

python3 tooling/agent-harness/control_plane.py jira \
