...
```bash
python3 agent-harness/runner.py <packet.json> \
  --provider codex \
...
`agent-harness/design.py` reuses the same bounded local Codex/Claude CLI command builders but supplies a dedicated structured output schema. Spec/architecture grills and prototype evaluation are read-only; only `prototype-agent` receives a disposable workspace-write scratch worktree.
