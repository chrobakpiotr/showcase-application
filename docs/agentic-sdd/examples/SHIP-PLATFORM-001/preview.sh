...
python3 agent-harness/wayfinder.py validate "$MAP"
echo
echo "2/6 Status"
python3 agent-harness/wayfinder.py status "$MAP"
echo
echo "3/6 Leverage-ranked frontier"
python3 agent-harness/wayfinder.py frontier "$MAP"
echo
echo "4/6 Terminal reconciliation must block before discovery"
set +e
python3 agent-harness/wayfinder.py reconcile "$MAP"
rc=$?
set -e
if [[ "$rc" -ne 2 ]]; then
...
echo "  python3 agent-harness/eval.py run --suite shipping-preflight --repeat 1"
