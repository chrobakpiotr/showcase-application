...
python3 agent-harness/design.py "$FEATURE" --plan
echo
echo "2/5 Validate inactive maintained example"
python3 agent-harness/harness.py validate "$FEATURE"
echo
echo "3/5 Initially ready tasks"
python3 agent-harness/harness.py ready "$FEATURE"
echo
echo "4/5 Specialist routing"
for task in T-001 T-002 T-003; do
  echo "-- $task"
  python3 agent-harness/harness.py reviewers "$FEATURE" "$task"
done
echo
echo "5/5 Orchestration plan (side-effect free; maintained example is not an active spec)"
python3 agent-harness/orchestrate.py "$FEATURE" --plan
